...
/*
	 This application can encrypt and decrypt hex data based on the fish3
	 symmetric-key block cipher.
 */
use threefish::Threefish256;
use cipher::{BlockEncrypt, NewBlockCipher, BlockDecrypt};
...
fn main() {
	...
	if args.len() == 1 || args[1] == "-h" {
		println!("Usage: ./fish3 -e <data file>");
		println!("       ./fish3 -d <encrypted file> <key file>");
		return;
	}
	else if args.len() > 4 {
	...
	if cmd == "-d" {
		...
		let cipher = Threefish256::new((&k_vec[..32]).into());
		let mut mark = 0; 
	...
	if cmd == "-e" {
		...
		let cipher = Threefish256::new(&(key.into()));
...
/*
	remove_trail will remove the trailing zeros at the end
...
fn remove_trail(vec: &Vec<u8>) -> Vec<u8> {
	let res = match std::str::from_utf8(&vec) {
                Ok(val) => val,
                  Err(err) => {
                    println!("Unable to convert slice to string. Error : {}",err);
                    return Vec::new();
                  }
              };
 ...
 */
fn encrypt(buf:&mut Vec<u8>, cipher: Threefish256) {
