...
/*
	 This application can encrypt and decrypt data based on the fish_2
	 symmetric-key block cipher.
*/
use twofish::Twofish;
use twofish::cipher::{NewBlockCipher, BlockEncrypt, BlockDecrypt};
...
fn main() {
	...
	if args.len() == 1 || args[1] == "-h" {
		println!("Usage: ./fish2 -e <data file>");
		println!("       ./fish2 -d <encrypted file> <key file>");
		return;
	}
	else if args.len() > 4 {
	...
	if cmd == "-d" {
		...
		let cipher = Twofish::new((&k_vec[..32]).into());
		let mut mark = 0; 
	...
	if cmd == "-e" {
		...
		let cipher = Twofish::new(&(key.into()));
...
/*
	remove_trail will remove the trailing zeros in a decrypted
	message.
*/
fn remove_trail(vec: &Vec<u8>) -> Vec<u8> {
	let res = std::str::from_utf8(&vec).unwrap();
...
fn read_data(in_file: &str) -> Vec<u8> {
	...
	let mut file = match File::open(in_file) {
		Ok(f) => f,
			Err(err) => {
				println!("Unable to open specified data file \"{}\". Error: {}",in_file,err);
				return Vec::new();
			}
	};
	let mut buf = Vec::new();
	match file.read_to_end(&mut buf) {
		Ok(r) => r,
			Err(err) => {
				println!("Unable to read bytes in data file. Error: {}",err);
				return Vec::new();
			}
	};
	buf
...
fn read_enc(enc_file: &str) -> Vec<u8> {
	...
	match file.read_to_end(&mut buf) {
		Ok(r) => r,
			Err(err) => {
				println!("Unable to read encrypted data bytes. Error: {}",err);
				return Vec::new();
			}
	};
	buf
 ...
 */
fn encrypt(buf:&mut Vec<u8>, cipher: Twofish) {
...
fn read_key(file : &str) -> Vec<u8> {
	let mut k_file = match File::open(file) {
		Ok(f) => f,
			Err(err) => {
				println!("Unable to open key.txt file. Error: {}",err);
				return Vec::new();
			}
	};
	...
	match k_file.read_to_end(&mut key_vec) {
		Ok(nb) => nb,
			Err(err) => {
				println!("Unable to read bytes in key.txt. Error: {}",err);
				return Vec::new();
			}
	};
	key_vec
