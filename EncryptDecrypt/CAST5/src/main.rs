...
/*
	 This application can encrypt and decrypt hex data based on the CAST5
	 symmetric-key block cipher.
 */

use rand_core::{RngCore,OsRng};
use std::fs::File;
use cast5::cipher::{BlockEncrypt, BlockDecrypt, NewBlockCipher};
use cast5::Cast5;
use std::env;
use std::io::Read;
use std::io::Write;
...
fn main() {
	...
	if args.len() == 1 || args[1] == "-h" {
		println!("Usage: ./CAST5 -e <data file>");
		println!("       ./CAST5 -d <encrypted file> <key file>");
		return;
	}
	else if args.len() > 4 {
	...
	if cmd == "-d" {
		...
		let cipher = Cast5::new((&k_vec[..16]).into());
		let mut mark = 0; 
...
/*
	remove_trail will remove the added trailing zeros from the final
	output.
*/
 ...
 */
fn encrypt(buf:&mut Vec<u8>, cipher: Cast5) {
