...
use kuznyechik::cipher::NewBlockCipher;
use kuznyechik::cipher::BlockDecrypt;
use kuznyechik::cipher::BlockEncrypt;
...
fn main() {
	...
	if args.len() == 1 || args[1] == "-h" {
		println!("Usage: ./kuznyechik -e <file for encrypting>");
		println!("       ./kuznyechik -d <encrypted file> <key file>");
		return;
	}
	else if args.len() > 4 {
	...
	if cmd == "-d" {
		...
		let cipher = Kuznyechik::new((&k_vec[..32]).into());
		let mut mark = 0; 
...
fn encrypt(buf:&mut Vec<u8>, cipher: Kuznyechik) {
