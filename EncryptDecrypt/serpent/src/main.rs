...
/*
	 This application can encrypt and decrypt hex data based on the Serpent
	 symmetric-key block cipher.
 */

use serpent::Serpent;
use serpent::cipher::NewBlockCipher;
use serpent::cipher::BlockDecrypt;
use serpent::cipher::BlockEncrypt;
...
fn main() {
	...
	if args.len() == 1 || args[1] == "-h" {
		println!("Usage: ./Serpent -e <data file>");
		println!("       ./Serpent -d <encrypted file> <key file>");
		return;
	}
	else if args.len() > 4 {
	...
	if cmd == "-d" {
		...
		let cipher = Serpent::new((&k_vec[..16]).into());
		let mut mark = 0; 
	...
	if cmd == "-e" {
		...
		let cipher = Serpent::new(&(key.into()));
...
/*
	remove_trail removes trailing zeros in a decrypted message.
*/
fn remove_trail(vec: &Vec<u8>) -> Vec<u8> {
	let mut res = match std::str::from_utf8(&vec) {
                    Ok(res) => res,
                      Err(err) => {
                        println!("Unable to convert slice to string slice. Error: {}",err);
                        return Vec::new();
                      }
                  };
 ...
 */
fn encrypt(buf:&mut Vec<u8>, cipher: Serpent) {
