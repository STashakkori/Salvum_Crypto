pub mod rustyham {
    ...
    pub fn hamming(action: Hamming, s: String) -> String {
        match action {
            Hamming::Encode | Hamming::EncodeBinary => {
                ...
                let len = 2u32.pow(lenpow) - 1;

                // the thing we're storing the hamming code in
                let mut code: Vec<bool> = repeat(false).take(len as usize).collect();

                // set data bits
                for i in 1..len {
                    if (i & (i - 1)) != 0 {  // if i is not a power of 2
                        code[i as usize -1] = bytes_iter.next().unwrap_or('0') == '1';
                    }
                }

                // set parity bits
                for i in 0..lenpow {
                    code[2u32.pow(i) as usize - 1] = calc_parity(&code, i);
                }
    ...
    fn calc_parity(code: &Vec<bool>, i: u32) -> bool {
        ...
        for j in bi..code.len() as u32 {
            if !ignore && code[j as usize] { parity = !parity }
            counter += 1;
